...
Original file is located at
    https://colab.research.google.com/drive/1i3qB_ds-tIx-h-bv2B-E2t_JDFdtjueB
"""
...
for i in range(len(ti_s_pat)):
    ...
    for j in range(len(ti_s_per)):
        ...
        if (diff<t):
            p1_lat=df_pat['lat'][i]
            p1_long=df_pat['long'][i]
            p2_lat=df_per['lat'][j]
            p2_long=df_per['long'][j]
            p1=(p1_lat,p1_long)
            p2=(p2_lat,p2_long)
            dist_lam = lambda c1,c2:round(distance.distance(c1,c2).m,2)
            dist=dist_lam(p1,p2)
            if dist<d:
                pat_id.append([p1_lat])
                pat_id.append([p1_long])
                pat_id.append(df_per['id'][j])
                pat_id.append([p2_lat])
                pat_id.append([p2_long])
                master.append(pat_id)
df_mas = pd.DataFrame(master,columns=['Source','LAT','LONG','Contact','LAT','LONG'])
